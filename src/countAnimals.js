...
function countAnimals(animal) {
  ...
  const { specie: name, genger } = animal; // O paramentro recebido é um objeto com duas possibilidades, desestruturamos para conseguir ter acesso as keys desse objeto.
  if (name && !genger) {
  ...
  return species.find((onesAnimal) => onesAnimal.name === name)
    .residents.filter((sexAnimal) => sexAnimal.sex === genger).length; // O Filter entrega um array com os elementos que satisfazem a condição.
}
