...
function createObject(object) {
  const animals = object.responsibleFor.map((id) => species.find((animal) => animal.id === id).name);
  return {
    id: object.id,
    fullName: `${object.firstName} ${object.lastName}`,
    species: animals,
    locations: animals.map((animal) => species.find((location) => location.name === animal).location)
  };
}
